...
And because we are working with a potential with $\sigma = 1$, we need to expand the box, and set \code{box\_width = 10.0} instead of 1.
...
\end{lstlisting}
    ...
    \item After the initialization of the constant, insert the loop over \code{range(n\_steps)}, but this time we should use the \code{velo\_verlet} function, instead of \code{simulate\_step}.
