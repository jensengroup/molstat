...
\begin{enumerate}
    \setcounter{enumi}{2}
    \item Familiarize yourself with the Numpy methods \code{arange}, \code{sum}, \code{mean} and \code{pi}.
\end{enumerate}

%% Whole part seems out of place and a bit trivial since arange already have been introduced
...
velocities, box width etc. The \code{initialize\_particles()} function you
will import is a bit more advanced.
The new \code{initialize\_particles()} function takes as arguments the number of
...
%Now it's time to test the \code{initialize\_particles()} function.
%
%
%\begin{enumerate}
%    \setcounter{enumi}{1}
...
\begin{enumerate}
    ...
    \item Make three empty lists called Ekin\_list, Epot\_list and Etot\_list and append
        the current values to these lists every 10 steps instead of printing them.

    \item Run your simulation for 10,000 steps. Do not save a video, since this
        will take several minutes to compile. Use Matplotlib to make a plot of the
        energies vs. time:
...
\end{lstlisting}
    ...
    \item Like for the energies, make an empty list called T\_list and
    append the instantaneous temperature every 10 steps. Use Matplotlib to make a plot of
    this list. See Figure \ref{fig:temperature} for an example.
    ...
    % Have no idea what the answer to this question is lol
    ...
    \item Use Matplotlib to plot the pressure during the simulation, as done in the
