...
\code{cd <directory>                   \# enter the folder called <directory>} \\
\code{cd ..                            \# navigate to the parent directory.}\\
\code{ls                               \# print the contents of the current directory}\\
\code{mkdir <directory>                \# creates an empty folder called <directory>}\\
\code{cp <file1> <file2>               \# copy <file1> to <file2>}\\
\code{cp <file> <directory>            \# copy <file> to <directory>}\\
\code{cp -r <directory1> <directory2>  \# copy <directory1> to <directory2>.}\\
\code{mv <file1> <file2>               \# move <file1> to <file2>} \\
...
\begin{enumerate}
    ...
    \begin{centering}
      \code{5+9, 5-9, 5*9, 5/9, 5+2*9, 5.0+2.0*9.0, 5.0/9 5**2, 5\%2}
    \end{centering}
\end{enumerate}
...
\begin{figure}[htb]
  \centering
  \subfloat[]{\includegraphics[width=0.45\textwidth]{images/coordinates_start.eps} \label{fig:firstbox}}
  \subfloat[]{\includegraphics[width=0.45\textwidth]{images/coordinates_start_full.eps} \label{fig:secondbox}}
  \caption{
