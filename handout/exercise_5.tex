...
\begin{lstlisting}
    2.4918 497.58 0.4790
\end{lstlisting}
where the first column is the transition energy in electron volt (eV), the second is the energy in nm/the wavelength of the transition and the third is the oscillator strength of the transition (unit less).\\
...
\begin{equation}
\epsilon(\tilde{\nu})=\sum_{j=1}^n \epsilon_j(\tilde{\nu})=\sum_{j=1}^n  \frac{\text{N}_A\si{e^2}}{2\ln(10) \text{m}_e \si{c^2}  \epsilon_0}\frac{f^j}{\sigma}\sqrt{\frac{\ln(2)}{\pi}} \exp{\Bigg(-4 \ln(2) \left[\frac{\left(\frac{1}{\lambda}-\frac{1}{\lambda_{if}^j}\right)}{\sigma\cdot 10^{-7}}\right]^2}\Bigg) \si{M^{-1}cm^{-1}} \label{eq_uv_vis}
\end{equation}

where $f^j$ and $\lambda_{if}^j$ are the oscillator strength and wavelength related to the $\text{j}^{th}$ transition and $n$ is the total number of transitions in the dataset. Furthermore, $N_A$ is avogadros number, $e$ is the elemental charge, $m_e$ is the electron mass, $c$ is the speed of light and $\epsilon_0$ is the vacuum permittivity.  $\sigma$ is the full width half maximum of the Gaussian function used to convolute the UV-vis spectra. It defines the width of the absorption peaks. In this example we will use $\sigma = 3226 \si{cm^{-1}}$.
\begin{enumerate}
...
\end{lstlisting}
\item Define a constant $k=\frac{\text{N}_A\si{e^2}}{2\ln(10) \text{m}_e \si{c^2}  \epsilon_0}\sqrt{\frac{\ln(2)}{\pi}}\cdot 10^{-1} \frac{\si{L}}{\si{mol}}\si{cm^{-2}}$ and use all the variables you just defined to calculate the value of $k$. Print the value of \code{k}. If you have done this correctly you should find
\code{k=217512853.406}
\end{enumerate}
In science we normally don't want to present a number with this many significant digits. 
\begin{enumerate}[resume]
\item Print k again, but this time in scientific notation as a 4 digit number using \code{.format(k)}. \textit{Hint} Use Google if you are not familiar with \code{.format()}
\end{enumerate}
Now we are ready to calculate the UV-vis spectra. 
\begin{enumerate}[resume]
\item Load all the wave lengths into a list called \code{lambda\_list}, and oscillator strengths into a list called \code{osillator\_list}
\item Make a third list \code{x=[]} starting at 200 and ending at 700 with length 500 - these are all the points at which you want to calculated the total absorption, given as $\lambda$ in \eqref{eq_uv_vis}. \textit{Hint} use \code{np.linespace()}
\item Define a function that calculates the total absorption are each $\lambda$. You can use the following bit of code as inspiration
\begin{lstlisting}
def uvvis(x,l,f):
	...
	for j in range(1,len(x)):
		for i in range(0,len(l)):
			lambda_1[i]=....
		lambda_tot[j]=sum(lambda1)
	return lambda_tot

\end{lstlisting}

\item Plot the result - remember to add legend, labels and title to the plot with units on the axis \textit{Hint} What is printet on the y-axis when you use \verb+plt.ylabel(r'$\varepsilon$')+ ?
