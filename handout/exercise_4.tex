...
Before you start make you you have downloaded \code{md\_header.py} and \code{md\_video.py}.
Also make sure that you have downloaded Anaconda, by writing the following into a terminal
\begin{lstlisting}
wget -q -O - http://dgu.ki.ku.dk/molbox/setup.sh | bash
\end{lstlisting}

Firstly we want to use the \code{md\_header.py} for the initialization and calculation of the LJ force, which is done by importing it;
...
The \code{initialize\_particles} function from the header takes the arguments
number of atoms, temperature, density (rho) and eps matrix (which is still 1).
Eps is now a matrix because then we get to play with multiple eps values later.
The function returns the positions, velocities, forces and the box width, based on the temperature and density.
...
It is an $Nx2$ array, which holds the $x$ and $y$ coordinates for all $N$ particles.
...
The second function from the md module is the \code{lennard\_jones} which takes the arguments positions, box width and eps (where eps is the matrix defined earlier).
The function returns the potential energy, the force and the 
...
\begin{enumerate}[resume]
    \item Understand the variables energy\_potential and F.
        What are the dimensions and data types of the variables?

 
    \item Create a function called "simulation" with the following parameters;
        number of atoms,
        number of steps,
...
\end{lstlisting}
    ...
    \item What happens to the energy?

    \item Make a video of a gas, liquid and solid simulation.
